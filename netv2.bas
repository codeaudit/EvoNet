...
sub ENet.init(veclen as ulong,density as ulong,depth as ulong,precision as ulong,hash as ulong=0)
	...
	this.hash=hash
	sizememory()
	dim as ulongint nm=veclen*density*(depth-1)
	dim as single sc=sqr(1.0/depth)
	for i as longint=0 to nm-1
		wts[i]=xor128.rndSingleSym()
	next
	for i as longint=nm to veclen*density*depth-1
		wts[i]=xor128.rndSingleSym()*sc
	next
	parentCost=1!/0!
end sub
...
sub ENet.recall(result as single ptr,invec as single ptr)
	...
	for j as ulongint=0 to depth-1	
		...
		for i as ulongint=0 to density-1
			rp.hashflipA(workA,workA,h,veclen)
			rp.wht(workA,veclen)
			rp.multiply(workC,workA,workA,veclen)  'just take the square of the elements
			rp.hashflipA(workC,workC,not h,veclen) 'workA to create a point wise sparse vector
			rp.wht(workC,veclen)				    'that you spread out to a pattern wise
			rp.multiplyaddto(result,workC,wtidx,veclen) 'sparse vector 
			wtidx+=veclen
			h+=1
		next
